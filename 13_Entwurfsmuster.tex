...
\definition[1]{Entkopplungs-Muster}{Entkopplungsmuster \important{teilen} ein System in \important{mehrere Einheiten}, so dass einzelne Einheiten unabhängig voneinander erstellt, verändert, ausgetauscht und wiederverwendet werden können. Der Vorteil von Entkopplungsmustern ist, dass ein System durch \important{lokale Änderungen} verbessert, angepasst und erweitert werden kann, \important{ohne das ganze} System zu modifizieren.}
% Adapter
\subsubsection{Adapter}
\definition[0]{Adapter}{Passt die \important{Schnittstelle} einer Klasse an \important{eine Andere} von ihren Klienten erwartete Schnittstelle an.}
...
\definition[0]{Beobachter}{Ein Beobachter \important{beobachtet} ein Objekt und \important{informiert} andere Objekte \important{über Änderungen} an diesem.}
\anmerkung[0]{}{Ein Beobachter definiert eine \important{1-zu-n Abhängigkeit} zwischen Objekten, so dass die \important{Änderung} eines Zustandes eines Objektes dazu führt, dass alle abhängigen Objekte \important{benachrichtigt} und automatisch \important{aktualisiert} werden.}
\newline
...
\anmerkung[0]{Konsequenzen}{Beobachter können neu hinzugefügt oder entfernt werden, ohne das Subjekt oder andere Beobachter zu ändern}
 
\subsubsection{Brücke / Brügge / Bridge}
\definition[0]{Brücke}{Die Brücke \important{trennt Abstraktion} von ihrer \important{Implementierung}, sodass beide \important{unabhängig} voneinander \important{variiert} werden können.}
...
\anmerkung[0]{}{Robust ist der Iterator, weil er \important{gleichzeitig mehrere Traversierungen} ermöglicht. Jeder Iterator enthält eine eigene „Laufvariable“.}
...
\definition[0]{Memento}{Erfasst und \important{externalisiert} den \important{internen Zustand} eines Objekts, ohne seine Kapselung zu verletzen, so dass das Objekt später in diesen Zustand zurückversetzt werden kann.}
